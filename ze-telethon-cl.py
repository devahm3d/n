...
async def handler(event):
    data = event.data.decode('utf-8')
    if data == 'ze':
        await bot.send_message(OWNER_ID, f"""**∘ بوت دارك لتجميع النقاط واوامر اخرى 

∘ مطور ومبرمج البوت دارك - @A7_M3
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('رشق مشاهدات ⟐', 'akbo')],
         ...
         [Button.inline('༺ 𝑆𝑂𝑈𝑅𝐶𝐸 𝐷𝐴𝑅𝐾 🖤 𝐁𝐎𝐓 ༻', 'button0')]
        ]
...
@bot.on(events.NewMessage(pattern='/python', from_users=5449190469))
